...
/*   Updated: 2018/03/19 15:01:51 by astadnik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_pos(const t_data data, t_point cur)
{
	...
	offs.y = -1;
	inters = 0;
	while (++offs.y < data.tok_height && (offs.x = -1))
		while (++offs.x < data.tok_width)
		{
			if (data.token[offs.y * data.tok_width + offs.x] == '.')
				continue ;
			if (cur.y + offs.y < 0 || cur.x + offs.x < 0 || cur.x + offs.x >=
					data.width || cur.y + offs.y >= data.height)
				return (0);
			p = ft_tolower(data.field[(cur.y + offs.y) * data.width
					+ cur.x + offs.x]);
			if (p == '.')
				continue ;
...
static ssize_t	pifagor(t_data data, size_t y, size_t x)
{
	...
	min = ft_pow(data.height * data.width, 2);
	cur.y = -1;
	while (++cur.y < data.height)
	{
		...
		{
			if (ft_tolower(data.field[cur.y * data.width + cur.x])
					== data.opponent)
			{
...
char			algo(const t_data data)
{
	...
	ft_printf("%d %d\n", best.y, best.x);
	return (min != (size_t)ft_pow(data.height * data.width, 2));
}
